function marikSAMloop2(time,fold,fn,pat)
% GLOBAL 
if ~exist('fn','var')
    fn='';
end
...
cd ..
if ~exist([fold,'/SAM/ictal,20-70Hz,Global.wts'],'file')
    eval(['!SAMcov64 -d ',fn,' -r ',fold,' -m ictal -v'])
    eval(['!SAMwts64 -d ',fn,' -r ',fold,' -m ictal -c Global -v'])
end
cd (fold)
cd SAM
[~,~, ActWgts]=readWeights('ictal,20-70Hz,Global.wts');
cd ..
