function extrema=aliceCompByMaxChMaxPeak(file,xlim,posSide,chans)
...
for sub=1:8
    ...
    [maxL(1,sub),maxI(1,sub)]=max(Gavg.individual(sub,labelI,samp)');
...
for sub=1:8
    samp=nearest(Gavg.time,xlim(sub));
    [maxR(1,sub),maxI(1,sub)]=max(Gavg.individual(sub,labelI,samp)');
