    ...
    for subi=1:22
        ...
        for foi=F
            ...
            for trli=1:length(corri)
                ...
                missAmp(1:248,trli,foi)=range([datafinal.trial{missi(trli)}(:,sBL:s0)-filt]')./2;
            end
        end
        for foi=1:length(F)
        ...
        CorrAmp(1:248,subi,:)=mean(corrAmp,2);
        disp(['done ',folder])
        cd ..
    end
    save (['res5_',suf,'.mat'],'F','toi','CorrR','CorrPh','CorrAmp','MissR','MissPh','MissAmp')
end
