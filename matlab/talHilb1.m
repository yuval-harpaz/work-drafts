function dataHilb=talHilb1(subs,coords,coordType,label,pat);
% subs={'quad01'}
    ...
    if exist(['~/Desktop/talResults/CohSource/',sub],'file')
        ...
        for i=1:2 % two eyes closed per subject
            ...
            for trgi=startTrg:2:endTrg
                ...
                strOnset=num2str(trOnset); %#ok<NASGU>
...
%             sdata=ft_convert_units(sdata,'mm');
            ...
            for triali=1:length(dataHilb.trial)
                ...
                if triali==1
                    vs=dataHilb.trial{1,1};
                else
                    vs=[vs(1,:) dataHilb.trial{1,triali}(1,:);vs(2,:) dataHilb.trial{1,triali}(2,:)];
                end
                avgs(triali,1:2)=mean(dataHilb.trial{1,triali}(:,1:509),2);
                avgs(triali,3:4)=mean(dataHilb.trial{1,triali}(:,509:1017),2);
            end
            EC=corrcoef(vs');
            AEC=EC(1,2) % note here there is no average so it is not realy AEC
            rows=size(avgs,1);
        ...
        save([pat,'/talResults/Hilbert/',sub,'_h1_',label{1,1},'_',label{1,2}],'AEC1','AEC2','CAE1','CAE2')
end
cd(PWD);
end
