function [P,figure1]=aquaStatPlot_rm(data1,data2,data3,data4,data5,data6,titles,xlim,zlim,chans,midline,pthr); %#ok<INUSD>
...
for chani=1:length(chans)
    ...
    aqua=[data1.powspctrm(:,chi,xlim(1)),data4.powspctrm(:,chi,xlim(1))];
    quad=[data2.powspctrm(:,chi,xlim(1)),data5.powspctrm(:,chi,xlim(1))];
    verb=[data3.powspctrm(:,chi,xlim(1)),data6.powspctrm(:,chi,xlim(1))];
    
    %[p, table] = anova_rm(dyslexia)
    %[p, table] = anova_rm(control)
    %[p, table] = anova_rm({dyslexia control});
    p = anova_rm({aqua quad verb},'off');
    P(chi,:)=p([1 2 4]); % Time Group Interaction
...
dataavg=data1;
dataavg.powspctrm=(mean(data1.powspctrm)+mean(data2.powspctrm)+mean(data3.powspctrm)+mean(data4.powspctrm)+mean(data5.powspctrm)+mean(data6.powspctrm))/6;
...
for vi=1:2
    for gri=1:3
        datai=counter+gri
        subplot(2,3,datai)
        eval(['ft_topoplotER(cfg,data',num2str(datai),');'])
        title(titles{datai});
    end
    counter=counter+3;
end
