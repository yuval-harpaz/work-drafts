function extrema=aliceCompLat(file,xlim,posSide,chans)
...
for sub=1:8
    ...
    maxPerCh=max(subdata');
    [maxL(1,sub),maxI(1,sub)]=max(maxPerCh);
    maxChL(1,sub)=Gavg.label(labelI(maxI(1,sub)));
    [~,latI]=max(subdata(maxI(1,sub),:));
    maxLatL(1,sub)=Gavg.time(latI+samp-win+1);
    minPerCh=min(subdata');
    [minL(1,sub),minI(1,sub)]=min(minPerCh);
    minChL(1,sub)=Gavg.label(labelI(minI(1,sub)));
    [~,latI]=min(subdata(minI(1,sub),:));
    minLatL(1,sub)=Gavg.time(latI+samp-win+1);
end
%% right max and min
...
for sub=1:8
    ...
    subdata=squeeze(Gavg.individual(sub,labelI,samp-win:samp+win));
    maxPerCh=max(subdata');
    [maxR(1,sub),maxI(1,sub)]=max(maxPerCh);
    maxChR(1,sub)=Gavg.label(labelI(maxI(1,sub)));
    [~,latI]=max(subdata(maxI(1,sub),:));
    maxLatR(1,sub)=Gavg.time(latI+samp-win+1);
    minPerCh=min(subdata');
    [minR(1,sub),minI(1,sub)]=min(minPerCh);
    minChR(1,sub)=Gavg.label(labelI(minI(1,sub)));
    [~,latI]=min(subdata(minI(1,sub),:));
    minLatR(1,sub)=Gavg.time(latI+samp-win+1);
end
cfg=[];
...
extrema.maxLatR=maxLatR;
extrema.maxLatL=maxLatL;
extrema.minLatR=minLatR;
extrema.minLatL=minLatL;
