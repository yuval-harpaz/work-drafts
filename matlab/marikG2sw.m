function marikG2sw(time,fold,fn,pat)
% ADAPTIVE
if ~exist('pat','var')
...
%         end
        if exist('./slideg1+orig.BRIK','file')
            !rm slideg1+*
        end
...
%         !rm slide1+*
        ...
        cfg.prefix='slideg1';
        VS2Brik(cfg,g2seg);
        eval(['!3dcalc -a ../maskRS+orig -b slideg1+orig -exp ''','b*ispositive(a)''',' -prefix g2sw',num2str(segi),'_',num2str(counters(segi))]);
        !rm slideg1+*
...
for triali=1:ntrl
    ...
    TI=num2str(triali);
    eval(['!3dcalc -a g2sw',TI,'_1+orig -b g2sw',TI,'_2+orig -c g2sw',TI,'_3+orig -d g2sw',TI,'_4+orig -e g2sw',TI,'_5+orig -exp ''','(a+b+c+d+e)/5''',' -prefix ',num2str(time(1)+triali-1),'g2sw']);
end
str=['!3dTcat -prefix g2sw',fold,' '];
for triali=time(1):time(2)
    str=[str,num2str(triali),'g2sw+orig '];
end
eval(str);
