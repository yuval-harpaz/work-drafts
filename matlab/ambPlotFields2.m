...
avgD=avgDre;
avgD.avg=10^12*(avgDre.avg+avgSre.avg)./2; %Re really
col=[0:0.1:1];
map=[col;col;col];
cfg=[];
...
subplot(2,3,2);
topoplotER(cfg,avgD);
title('Related','FontName','Times','FontSize',30,'Position',[0 titlepos 1]);
subplot(2,3,5);
...
annotation('textbox',[0.65 0.8 0.1 0.08],...
    ...
    'LineStyle','none');
avgS=avgD; 
avgS.avg=10^12*(avgSur.avg+avgDur.avg)./2;
cfg.xlim = [0.151 0.234];
subplot(2,3,1);
topoplotER(cfg,avgS);
title('Unrelated','FontSize',30,'FontName','Times','Position',[0 titlepos 1]);
subplot(2,3,4);
cfg.xlim = [0.234 0.387];
topoplotER(cfg,avgS);
...
%% %% diff in fT
avgDiff=avgD;
avgDiff.avg=10^12*(avgSur.avg+avgDur.avg-avgSre.avg-avgDre.avg)/2;
cfg.xlim = [0.151 0.234];
cfg.zlim=[-0.01 0.01];
...
title('UR-Re','FontSize',30,'FontName','Times','Position',[0 0.7 1]);
