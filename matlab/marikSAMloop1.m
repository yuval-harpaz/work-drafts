function marikSAMloop1(time,fold,fn,pat)
% SEGMENTS
if ~exist('pat','var')
...
%     cd segments/SAM
    ...
    cfg.prefix='seg1';
    VS2Brik(cfg,pow);
    eval(['!3dcalc -a ../maskRS+orig -b seg1+orig -exp ''','b*ispositive(a)''',' -prefix ',num2str(tt(segi)),'seg']);
    !rm seg1+*
    
end

str=['!3dTcat -prefix seg',fold,' '];
for triali=time(1):time(2)
    str=[str,num2str(triali),'seg+orig '];
end
eval(str);
%eval(['!3dTcat -prefix adapt5s',fold,' *all+orig.BRIK'])
eval(['!3drefit -Torg ',num2str(time(1)),' seg',fold,'+orig'])
% 
% 
