function [critT,critClustSize]=alicePermutePairedF(prefix1,prefix2,freq,tThresh)
% setA-setB
...
for permi=1:length(M)
    ...
    setB='-setB r2 ';
    for subi=1:8
        setA=[setA,sf{subi},'r1 "',conds{M1(permi,subi)},'_',num2str(subi),'+tlrc[',num2str(freq-1),']" '];
        setB=[setB,sf{subi},'r1 "',conds{M2(permi,subi)},'_',num2str(subi),'+tlrc[',num2str(freq-1),']" '];
    end
...
setB='-setB r2 ';
for subi=1:8
    setA=[setA,sf{subi},'r1 "',conds{M1(1,subi)},'_',num2str(subi),'+tlrc[',num2str(freq-1),']" '];
    setB=[setB,sf{subi},'r1 "',conds{M2(1,subi)},'_',num2str(subi),'+tlrc[',num2str(freq-1),']" '];
end
command = ['~/abin/3dttest++ -prefix ',prefix1,'_',prefix2,num2str(freq),'Hzttest -no1sam -paired -mask ~/SAM_BIU/docs/MASKctx+tlrc ',setA,setB];
eval(['!',command])
%[~, ~] = unix(command);

% dig in results
eval(['!~/abin/3dBrickStat -min -max ',prefix1,'_',prefix2,num2str(freq),'Hzttest+tlrc[1] > tMinMaxReal.txt'])
tReal=importdata('tMinMaxReal.txt');

% compute volume of largest positive and negative clusters
eval(['!~/abin/3dcalc -a ',prefix1,'_',prefix2,num2str(freq),'Hzttest+tlrc''','[1]''',' -exp ''','ispositive(a-',num2str(tThresh),')*a''',' -prefix pos'])
eval(['!~/abin/3dcalc -a ',prefix1,'_',prefix2,num2str(freq),'Hzttest+tlrc''','[1]''',' -exp ''','isnegative(a+',num2str(tThresh),')*a''',' -prefix neg'])
eval(['!~/abin/3dclust -quiet -1clip ',num2str(tThresh),' 5 125 neg+tlrc > negClust.txt'])
eval(['!~/abin/3dclust -quiet -1clip ',num2str(tThresh),' 5 125 pos+tlrc > posClust.txt'])
negClust=importdata('negClust.txt');
posClust=importdata('posClust.txt');
...
% cluster size (arrow6).
!rm TTnew+tlrc*
disp('!~/abin/afni -dset ~/SAM_BIU/docs/temp+tlrc &')
