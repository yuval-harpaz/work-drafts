...
for sfi=1:8
    ...
    cfg.feedback='no';
    %cfg.keeptrials='yes';
    megFFT = ft_freqanalysis(cfg, meg);
    ...
    cfg.frequency   = Afreq;
    cfg.keeptrials  = 'yes';      % keep single trials 
    srcRaw = ft_sourceanalysis(cfg, megFFT);
    cfg.frequency    = noiseFreq;
    noiseRaw=ft_sourceanalysis(cfg, megFFT);
    cfg=[];
    cfg.projectmom='yes';
    src.method='rawtrial';
    src=ft_sourcedescriptives(cfg, srcRaw);
    noise.method='rawtrial';
    noise=ft_sourcedescriptives(cfg, noiseRaw);
