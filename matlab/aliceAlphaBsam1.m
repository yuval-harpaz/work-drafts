function aliceAlphaBsam1(Afreq)
...
for sfi=1:8
    ...
    [~, ~, wts]=readWeights('SAM/alpha,7-13Hz,alla.wts');
    vsi=find(wts(:,1)~=0);
...
%     avgPSDn=avgPSD./repmat(mean(avgPSD(noiseF1:noiseF2,:)),size(avgPSD,1),1);
    fn=['rest1_',num2str(Afreq),'HzAwts'];
    %avgPSD=avgPSD*1e-8;
    ...
    eval(['!@auto_tlrc -apar ortho+tlrc -input ',fn,'+orig -dxyz 5'])
    eval(['!mv ',fn,'+tlrc.BRIK ~/Copy/MEGdata/alice/func/B/',fn,'_',num2str(sfi),'+tlrc.BRIK']);
    eval(['!mv ',fn,'+tlrc.HEAD ~/Copy/MEGdata/alice/func/B/',fn,'_',num2str(sfi),'+tlrc.HEAD']);    
    cd ..
end
