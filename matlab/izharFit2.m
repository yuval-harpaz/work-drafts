...
% end
% load('chan16.mat')
data=load ('NPX-Top-DP-10-2014-05-11-10-07-45-14-0001.mat',['CRAW_',chan]);
data=eval(['data.CRAW_',chan]);
sRate=44000;

time=1/sRate:1/sRate:(length(data)/sRate);
dataNotch=data;
for fi=50:50:300
...
fObj=fdesign.highpass('Fst,Fp,Ast,Ap',1,300,60,1,sRate);%
Filt=design(fObj ,'iir');
datahp = myFilt(dataNotch,Filt);
figure;
% plot(time,data,'r');
% hold on
%plot(time,dataNotch,'b')
plot(time,datahp,'k')
