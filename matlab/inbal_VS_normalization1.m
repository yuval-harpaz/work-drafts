function [vs , factor, prefix] = inbal_VS_normalization1(normalization_method, ActWgts, Data, BLtime, Covdir)
...
% normalization_method == 8 SAMspm; integrate noise covariance with weights
% normalization_method == 9 noise weights
%
% INPUTS
...
elseif normalization_method == 2
    ...
    onsetind = nearest(avgData.time,0); % YH -Data.cfg.trl(1,3) didn't work when trl is in cfg.previous
    BL1=nearest(avgData.time,BLtime(1));
    BL2=nearest(avgData.time,BLtime(2));
    vs = abs((mean_A - repmat(mean(mean_A(:,BL1:BL2),2),[1 size(mean_A,2)]))./repmat(std(mean_A(:,BL1:BL2),[],2),[1,size(mean_A,2)]));
...
elseif normalization_method == 4 
    ...
    covName=ls('*a.cov');
    doti=strfind(covName,'.');
    covNoSuf=covName(1:doti-1);
    if ~exist([covNoSuf,'.mat'], 'file') % if the file does not exist, we will create it
        eval(['!python ~/bin/readcov_inbal.py ',covName])
        Csig = load([covNoSuf,'.txt']);
        figure; subplot(1,2,1);imagesc(Csig);title([covNoSuf,' covariance matrix']);
        subplot(1,2,2);imagesc(Csig - Csig');title([covNoSuf,' - ',covNoSuf,'^T should be zerose']);
        save (covNoSuf,'Csig')
    else
        load(covNoSuf);
    end
    ...
    vs=abs(vs.*repmat(SNR(:),[1,size(vs,2)]));
    cd(PWD)
...
elseif normalization_method == 6
    ...
    SNR(find(isnan(SNR)))=0;
    factor = SNR;
    
    vs=abs(VSsignal.*repmat(SNR(:),[1,size(VSsignal,2)]));
    % figure; plot(SNR);
    ...
    % z calculation
    vs = (VSsignal - repmat(M(:),[1 size(VSsignal,2)]))./repmat(STD(:),[1,size(VSsignal,2)]);
    vs=abs(vs.*repmat(SNR(:),[1,size(VSsignal,2)]));
    elseif normalization_method == 8 
    % SNR from the noise cov matrix and the Sum cov matrix
    prefix = 'SAMspm_';
    factor =[];
    % reading the Noise.Cov and Sum.Cov matrix
    ...
    cd ../..;
    % creating the normalization vector SNR
    Noise=zeros(length(ActWgts),1);
    for c=1:size(ActWgts,1)
        for i=1:248
            for j=1:248
                Noise(c) = Noise(c)+ActWgts(c,i)*Cns(i,j)*ActWgts(c,j);
            end
        end
    end
%     SNR = Signal./ Noise;
%     factor = SNR;
    ...
    vs=(ActWgts*avgData.avg);
    vs=abs(vs./repmat(Noise,[1,size(vs,2)]));
    cd(PWD)
    elseif normalization_method == 9
    % WTS: using mean(abs(wts))
    prefix = 'WTSnoise_';
    % norm vector
    cd SAM
    nsWtsName=ls('*Noise.wts')
    [~, ~, NseWgts]=readWeights(nsWtsName(1:end-1));
    cd ..
    wts = mean(abs(NseWgts),2);
